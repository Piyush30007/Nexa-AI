...
from app.services.embedding import embed_texts, get_embedding_dim
...
def qdrant_request(method: str, path: str, **kwargs):
    """Send a request to Qdrant Cloud REST API."""
    ...
    with logfire.span("Processing File", file=filename, source=source_type):
        try:
            ...
            if ext == "pdf":
                ...
                from app.ingestion.Loader.docx_loader import parse_docx
                full_text = parse_docx(file_path)

            elif ext == "pptx":
                from app.ingestion.Loader.pptx_loader import parse_pptx
                full_text = parse_pptx(file_path)
...
def run_universal_ingestion(
    ...
    with logfire.span(
        ...
        # Route to sub-folders or treat the whole directory as one source
        subdirs = [
            d
